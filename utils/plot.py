...
def plot_scaling_predictions(
    ...
    Args:
        ...
        plot_fitted_curves: bool, whether to plot the fitted curves.
        plot_difference: bool, whether to plot the difference between the two curves.
    ...
    if not compute_metrics_only:
        ...
        if plot_difference:
            train_df['difference'] = all_df['difference'].loc[train_df.index]

        if plot_scatter and not plot_difference:
        ...
        if plot_difference:
            sns.scatterplot(data=train_df, x=plot_x_name, y='difference', **train_style_kwargs)
            if test_df is not None and plot_test_scatter:
                test_df.loc[:, plot_x_name] = all_df[plot_x_name].loc[test_df.index]
                test_df.loc[:, 'difference'] = all_df['difference'].loc[test_df.index]
                sns.scatterplot(data=test_df, x=plot_x_name, y='difference', **test_style_kwargs)
...
def plot_weight_analysis(
        ...
        remove_y_ticks=True, figsize=8, rotationx=30, rotationy=0, cmap="coolwarm", v=2,
        task='', get_tiled_weights=False
    ):
    """Visualize the metric weights for different number of PCs used in the regression analysis."""
    if filter_model_family:
        eval_df = eval_df[eval_df['Model Family'].isin(filter_model_family)]
    ...
    for y_metric_name, y_metric_abbr in y_metrics.items():
        ...
        y_metric_dict.update({y: 'CoT' if 'cot' in y else 'Naive' for y in y_metrics.values()})
        ...
        if get_tiled_weights:
            return tiled_weights

        sns.heatmap(tiled_weights, annot=True, fmt='.2f', cmap=cmap, ax=ax, vmin=-v, vmax=v)
        
        ax.set_xticklabels(pca_metrics, rotation=rotationx)
        if remove_y_ticks:
            ax.set_yticks([])
        else:
            ax.set_yticklabels([len(x) for x in x_metrics_list], rotation=rotationy)
            ax.set_ylabel(y_metric_dict[y_metric_name], fontsize=12)
...
def plot_scaling_comparison_multi_metrics(
        ...
        metrics_for_difference=None, legend_offset=0.63
    ):
    ...
    for idx, x_metrics in enumerate(x_metrics_list):
        ...
        if metrics_for_difference is not None:
            #print(f'Difference: {metrics_for_difference[0]} - {metrics_for_difference[1]}')
            eval_df = eval_df.copy()
            eval_df['difference'] = eval_df[metrics_for_difference[0]] - eval_df[metrics_for_difference[1]]
        ...
        for i, y_metric_name in enumerate(y_metric_list):
            ...
            all_kwargs = {**analysis_setup_kwargs, **additional_kwargs, **specific_kwargs} # added for BBH
            metric, fit_results, all_df, annot_info = plot_scaling_predictions(
                eval_df, x_metrics, y_metric_name, 
               **all_kwargs
            )
        ...
        if plot_train_test_legend:
            ...
            leg1 = ax.legend(train_test_handles, train_test_labels, loc='upper left', ncol=2, fontsize=legend_fontsize, bbox_to_anchor=(0., legend_offset))

    return fig, all_fit_results, metric
