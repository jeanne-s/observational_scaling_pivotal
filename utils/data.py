...
def fit_multivariate_regression_model(
    ...
    if nonlinearity is None:  # linear regression
        ...
        elif reg_method == "lasso":  # L1 regularization (Lasso)
            def lasso_loss(y_true, y_pred, p, lambda_):
                residual = y_pred - y_true
                data_loss = 0.5 * np.sum(residual ** 2)
                reg_loss = lambda_ * np.sum(np.abs(p))
                return data_loss + reg_loss
            
            def lasso_objective_function(p, lambda_):
                y_pred = _func(X_train.values, *p)
                return lasso_loss(y_train.values, y_pred, p, lambda_)
            
            lambda_ = reg_kwargs.get("lambda", 0.1)
            method = "L-BFGS-B" if bounds is not None else "BFGS"
            
            result = minimize(lambda p: lasso_objective_function(p, lambda_), p0, method=method, bounds=bounds)
            popt = result.x

        elif reg_method == "ridge":  # L2 regularization (Ridge)
            def ridge_loss(y_true, y_pred, p, lambda_):
                residual = y_pred - y_true
                data_loss = 0.5 * np.sum(residual ** 2)
                reg_loss = lambda_ * np.sum(p ** 2)
                return data_loss + reg_loss
            
            def ridge_objective_function(p, lambda_):
                y_pred = _func(X_train.values, *p)
                return ridge_loss(y_train.values, y_pred, p, lambda_)
            
            lambda_ = reg_kwargs.get("lambda", 0.1)
            method = "L-BFGS-B" if bounds is not None else "BFGS"
            
            result = minimize(lambda p: ridge_objective_function(p, lambda_), p0, method=method, bounds=bounds)
            popt = result.x
...
def get_bbh_subtask_scaling_metrics(task: str = 'tracking_shuffled_objects_seven_objects',
                                    mmlu_subtasks: list = []
):
    from utils.constants import DEFAULT_PCA_PREPROCESS_KWARGS, MODEL_SIZE_METRIC, TRAINING_FLOPS_METRIC, PC_METRIC_NUM_3
    import seaborn as sns
    ...
    GSM8K_METHOD_SCALING_COT_METRIC_MAP = {
        f'bbh_fewshot_{task}_3_exact_match,flexible-extract': "Naive + Greedy",
        f'bbh_cot_fewshot_{task}_3_exact_match,flexible-extract': "CoT + Greedy",
    }
    ...
    GSM8K_METHOD_SCALING_METRICS = [
        f'bbh_fewshot_{task}_3_exact_match,flexible-extract',
        f'bbh_cot_fewshot_{task}_3_exact_match,flexible-extract',
    ...
    GSM8K_METHOD_SCALING_COLOR_MAP = {
        f'bbh_fewshot_{task}_3_exact_match,flexible-extract': color_palette[1],
        f'bbh_cot_fewshot_{task}_3_exact_match,flexible-extract': color_palette[0],
    }
    ...
    for e in GSM8K_METHOD_SCALING_METRICS:
        ...
        if e != f"bbh_fewshot_{task}_3_exact_match,flexible-extract":
            GSM8K_SETUP_SPECIFIC_KWARGS[e]["plot_adjust_kwargs"].update({"ylim": [-0.03, 0.97]})
        if e in [f"bbh_fewshot_{task}_3_exact_match,flexible-extract"]:
            GSM8K_SETUP_SPECIFIC_KWARGS[e]["grace_range_ratios"] = [0.05, 0.12]

    return GSM8K_COT_SCALING_METRICS, SCALING_SETUP_KWARGS, GSM8K_SETUP_SPECIFIC_KWARGS, GSM8K_METHOD_SCALING_ORIG_METRIC_MAP, GSM8K_METHOD_SCALING_COLOR_MAP
...
def load_acc_mmlu_subtasks(folder="/scratch2/jsalle/ObsScaling/eval_results/output"
    ...
    #mmlu_df.drop_duplicates(subset=['Model'], inplace=True) -> wrong because we may have multiple entries for the same model
    mmlu_df = mmlu_df.groupby('Model').mean()
    mmlu_df.reset_index(inplace=True)
    return mmlu_df


def merge_benchmarks_with_init_df(base_llm_eval_with_post_training,
                                  bench_df):
    merged_eval = pd.merge(base_llm_eval_with_post_training, bench_df, on='Model', how='outer')
    merged_eval = merged_eval[merged_eval['Model'].isin(bench_df['Model'])]

    # Remove also Qwen2 for now because it was not evaluated on GSM8K, ARC-C, etc.
    merged_eval = merged_eval[~merged_eval['Model'].str.contains('Qwen2')]
    return merged_eval


def regression_raw_benchmarks_difference_cot_naive_get_kwargs(bbh_task: str,
                                                              other_subtasks: list = [],
                                                              task_to_remove: str = None,
                                                              loss: str = "robust",
                                                              lambda_: float = 0.1
                                                              
):
    """ Get the kwargs for the regression of the difference between CoT and Naive for a 
    given BBH subtask. These knargs can then be used as input in the 
    `regression_raw_benchmarks_difference_cot_naive` function.
    """
    from utils.constants import DEFAULT_PCA_PREPROCESS_KWARGS, EVAL_BASE_MODEL_FAMILIES

    # metric_for_difference
    title_map = {
        f"bbh_fewshot_{bbh_task}_3_exact_match,flexible-extract,norm": "Naive - Greedy",
        f"bbh_cot_fewshot_{bbh_task}_3_exact_match,flexible-extract,norm": "CoT - Greedy",
        'difference': 'CoT - Naive'
    }
    metric_for_difference = list(title_map.keys())
    metric_for_difference.reverse()
    metric_for_difference = metric_for_difference[1:] # CoT - Naive

    # metric_list
    metric_list = ['ARC-C', 'HellaSwag', 'Winograd', 'TruthfulQA', 'XWinograd', 'HumanEval', 'GSM8K'] + other_subtasks
    if task_to_remove is not None:
        metric_list.remove(task_to_remove)

    # setup_kwargs
    bbh_pca_preprocess_kwargs = DEFAULT_PCA_PREPROCESS_KWARGS.copy()
    bbh_pca_preprocess_kwargs["imputation_metrics"] = metric_list
    bbh_pca_preprocess_kwargs["pca_metrics"] = metric_list
    ...
    # y_specific_kwargs
    title_map_keys = list(title_map.keys())
    y_specific_kwargs = {}
    for e in title_map_keys:
        y_specific_kwargs[e] = {
            "plot_adjust_kwargs": {"title": title_map[e], "ylabel": "Accuracy"},
        }

    # color_map
    color_palette = sns.color_palette()
    color_map = {
        f'bbh_fewshot_{bbh_task}_3_exact_match,flexible-extract,norm': color_palette[1],
        f'bbh_cot_fewshot_{bbh_task}_3_exact_match,flexible-extract,norm': color_palette[0],
        'difference': color_palette[3],
    }

    # Output: all regression_kwargs
    regression_kwargs = {
        'metric_for_difference': metric_for_difference,
        'metric_list': metric_list,
        'setup_kwargs': setup_kwargs,
        'y_specific_kwargs': y_specific_kwargs,
        'title_map': title_map,
        'color_map': color_map
    }

    return regression_kwargs


def regression_pca_safety_benchmarks(evals: pd.DataFrame,
                                     task_to_predict='wmdp'
):
    from utils.constants import DEFAULT_PCA_PREPROCESS_KWARGS, EVAL_BASE_MODEL_FAMILIES, PC_METRIC_NUM_3
    from utils.plot import plot_scaling_comparison_multi_metrics
    import copy

    USED_INSTRUCT_LLM_METRICS = ['MMLU', 'ARC-C', 'HellaSwag', 'Winograd', 'TruthfulQA', 'XWinograd', 'HumanEval', 'GSM8K']

    PCA_PREPROCESS_KWARGS = copy.deepcopy(DEFAULT_PCA_PREPROCESS_KWARGS)
    PCA_PREPROCESS_KWARGS["imputation_metrics"] = USED_INSTRUCT_LLM_METRICS
    PCA_PREPROCESS_KWARGS["pca_metrics"] = USED_INSTRUCT_LLM_METRICS

    SCALING_SETUP_KWARGS = {
        # Data preprocessing: PCA imputation and extraction
        **PCA_PREPROCESS_KWARGS,  # exclude GSM to avoid making the task trivial, e.g., using GSM to predict GSM
    ...
    METRIC_MAP = {
        f'{task_to_predict}': f'{task_to_predict}',
    }
    ...
    SCALING_COLOR_MAP = {
        f'{task_to_predict}': color_palette[1]
    }
    ...
    fig, all_fit_results, metric = plot_scaling_comparison_multi_metrics(
        evals, SCALING_METRICS, [PC_METRIC_NUM_3], 
        SCALING_SETUP_KWARGS, y_metric_specific_kwargs=SETUP_SPECIFIC_KWARGS,
        ymetric2title_map=SCALING_ORIG_METRIC_MAP, ymetric2color_map=SCALING_COLOR_MAP,
        plot_title=f'{task_to_predict}'
    )

    return all_fit_results[0]


def regression_raw_benchmarks_difference_cot_naive(base_llm_eval_with_post_training,
    ...
    fig, all_fit_results, metric = plot_scaling_comparison_multi_metrics(
        ...
        metrics_for_difference=regression_kwargs['metric_for_difference'],
        x_metrics_list=[regression_kwargs['metric_list']], 
        analysis_setup_kwargs=regression_kwargs['setup_kwargs'], 
        y_metric_specific_kwargs=regression_kwargs['y_specific_kwargs'],
        ymetric2title_map=regression_kwargs['title_map'], 
        ymetric2color_map=regression_kwargs['color_map'],
        plot_title="Difference CoT - Naive"
    )

    return all_fit_results[0]


def get_weights_from_fit_results(fit_results, 
                                 regression_kwargs: dict,
                                 norm_weights: bool = False
):
    """ Get the weights from the fit results of the regression. The weights are the coefficients
    of the linear regression model.
    """
    metric_list = regression_kwargs['metric_list']
    weights = fit_results['popt'][1:].reshape(1, len(metric_list))
    if norm_weights:
...
def get_full_post_training_df():
    ...
    mbpp_subtasks = ['mbpp']
    return merged_eval, mmlu_subtasks+mbpp_subtasks